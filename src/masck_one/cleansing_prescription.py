...
def resolve(profile,absolute: Envelope|None,factor_policy: FactorPolicy|None):
    ...
    if profile.decision=='REPEAT_OBSERVATION':return {**base,'status':'BLOCKED_CURRENT_OBSERVATION_CONTEXT'}
    if profile.decision not in ('PERSONALIZED','PERSONALIZED_WITH_VARIATION','CONSERVATIVE_REVIEW'):raise ControlError('unrecognized profile decision')
    ...
    chosen={k:factors[k][level] for k,level in proposal.items()}
    ...
    output=replace(absolute,policy_id=absolute.policy_id+'/'+factor_policy.policy_id,
