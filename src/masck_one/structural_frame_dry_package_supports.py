...
# Digital package-fit seeds only. These are not manufacturing tolerances.
...
class BatterySupportRail:
    ...
    def manifest(self) -> dict[str, object]:
        return {
            ...
            "interface_semantics": "REMOVABLE_KEYED_FRAME_RAIL_WITH_LATERAL_BATTERY_STOP",
            "measured": {
    ...
    def manifest(self, *, include_sha: bool = True) -> dict[str, object]:
        payload: dict[str, object] = {
            ...
            "assembly_sequence": [
                ...
                "insert each support key from posterior into its frame-side socket",
                "seat each shoulder against the positive insertion stop",
...
def _boss_and_socket(cx: float, zmin: float) -> tuple[cq.Workplane, cq.Workplane, float]:
    cz = zmin - (ROOT_BOSS_DEPTH_MM - ROOT_FRAME_CAPTURE_MM) / 2.0
    boss = (
        cq.Workplane("XY")
        .box(ROOT_BOSS_WIDTH_MM, ROOT_BOSS_HEIGHT_MM, ROOT_BOSS_DEPTH_MM, centered=(True, True, True))
        .translate((cx, 0.0, cz))
    )
    socket_cz = zmin - ROOT_SOCKET_DEPTH_MM / 2.0
    socket = (
        cq.Workplane("XY")
        .box(ROOT_SOCKET_WIDTH_MM, ROOT_SOCKET_HEIGHT_MM, ROOT_SOCKET_DEPTH_MM, centered=(True, True, True))
        .translate((cx, 0.0, socket_cz))
    )
    return boss, socket, socket_cz


def _rail(side_sign: float, root_x: float, socket_cz: float, battery_box: cq.BoundBox) -> cq.Workplane:
    key_w = ROOT_SOCKET_WIDTH_MM - 2.0 * ROOT_KEY_CLEARANCE_MM
    key_h = ROOT_SOCKET_HEIGHT_MM - 2.0 * ROOT_KEY_CLEARANCE_MM
    key_d = ROOT_SOCKET_DEPTH_MM - ROOT_KEY_CLEARANCE_MM
    key = (
        cq.Workplane("XY")
        .box(key_w, key_h, key_d, centered=(True, True, True))
        .translate((root_x, 0.0, socket_cz - ROOT_KEY_CLEARANCE_MM / 2.0))
    )
    shoulder_z = socket_cz - ROOT_SOCKET_DEPTH_MM / 2.0 - ROOT_SHOULDER_GAP_MM - ROOT_SHOULDER_THICKNESS_MM / 2.0
    shoulder = (
        cq.Workplane("XY")
        .box(ROOT_BOSS_WIDTH_MM, ROOT_BOSS_HEIGHT_MM, ROOT_SHOULDER_THICKNESS_MM, centered=(True, True, True))
        .translate((root_x, 0.0, shoulder_z))
    )
    battery_cz = (battery_box.zmin + battery_box.zmax) / 2.0
    post_height = abs(shoulder_z - battery_cz) + ARM_Z_MM
    post = (
        cq.Workplane("XY")
        .box(POST_WIDTH_MM, POST_Y_MM, post_height, centered=(True, True, True))
        .translate((root_x, 0.0, (shoulder_z + battery_cz) / 2.0))
    )
    ...
    arm = (
        ...
        .translate((arm_center_x, 0.0, battery_cz))
    )
    ...
    side_rail = (
        cq.Workplane("XY")
        .box(SIDE_RAIL_THICKNESS_MM, rail_y, rail_z, centered=(True, True, True))
        .translate((rail_center_x, (battery_box.ymin + battery_box.ymax) / 2.0, battery_cz))
    )
    return key.union(shoulder).union(post).union(arm).union(side_rail)


def build_structural_frame_dry_package_supports(
    *,
    model: MasckOneModel | None = None,
    reactions: StructuralFrameActuatorReactionArchitecture | None = None,
) -> StructuralFrameDryPackageSupportArchitecture:
    ...
    frame = base_frame
    root_data: list[tuple[float, float]] = []
    for side_sign in (-1.0, 1.0):
        root_x = side_sign * root_abs_x
        boss, socket, socket_cz = _boss_and_socket(root_x, float(frame_box.zmin))
        ...
        root_data.append((root_x, socket_cz))
    _single(frame, "frame with bilateral dry-package sockets")

    supports: list[BatterySupportRail] = []
    for index, (side_sign, (root_x, socket_cz)) in enumerate(zip((-1.0, 1.0), root_data, strict=True)):
        rail = _rail(side_sign, root_x, socket_cz, battery_box)
        _single(rail, SUPPORT_IDS[index])
        frame_intersection = _ivol(rail, frame)
        package_intersection = _ivol(rail, battery)
        protected = 0.0
        ...
        for item in model.protected_volumes.all:
            ...
            keepout = _protected_zone_solid(
                ...
                z_min_mm=float(rail_box.zmin) - 1.0,
                z_max_mm=float(rail_box.zmax) + 1.0,
            )
            protected += _ivol(rail, keepout)
        hostile = rail.translate((-side_sign * STOP_PROBE_MM, 0.0, 0.0))
        hostile_intersection = _ivol(hostile, battery)
        supports.append(
            BatterySupportRail(
                ...
                frame_intersection_mm3=round(frame_intersection, 8),
                package_intersection_mm3=round(package_intersection, 8),
                protected_intersection_mm3=round(protected, 8),
                hostile_package_stop_intersection_mm3=round(hostile_intersection, 8),
                rail=rail,
            )
        )

    return StructuralFrameDryPackageSupportArchitecture(
        source_reaction_architecture_sha256=reactions.architecture_sha256,
        frame_with_support_counterparts=frame,
...
def export_structural_frame_dry_package_supports(output_dir: Path) -> dict[str, object]:
    ...
    cq.exporters.export(
        architecture.frame_with_support_counterparts,
        str(output_dir / "structural_frame_with_battery_support_counterparts.step"),
    )
    ...
    (output_dir / "structural_frame_dry_package_supports_manifest.json").write_text(
