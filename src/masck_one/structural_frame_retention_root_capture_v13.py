...
"""V13: quantify axial free-play headroom and the tolerance recovery required.
...
explicit sensitivity contract so a later manufacturing-informed allocation can
recover a chosen reserve without changing nominal geometry.
"""
...
class StructuralFrameRetentionRootCaptureV13:
    ...
    def validate(self) -> "StructuralFrameRetentionRootCaptureV13":
        ...
        if self.source_capture_v12_sha256 != prior.evidence_sha256:
            raise StructuralFrameRetentionRootCaptureV13Error("source capture V12 evidence is stale")
        ...
        candidate_max = prior.maximum_axial_free_play_mm - required
        expected = tuple(round(v, 12) for v in (
            current,
            TARGET_AXIAL_FREE_PLAY_HEADROOM_MM,
            required,
            required,
            split,
            clip_candidate,
        ...
        actual = (
            self.current_headroom_mm,
            self.target_headroom_mm,
            ...
            self.candidate_clip_tolerance_mm,
            self.candidate_groove_tolerance_mm,
        ...
        payload = {
            "source_capture_v12_sha256": self.source_capture_v12_sha256,
            "values": actual,
...
def build_structural_frame_retention_root_capture_v13() -> StructuralFrameRetentionRootCaptureV13:
    ...
    values = tuple(round(v, 12) for v in (
        ...
        required,
        split,
        capture_v9.CLIP_AXIAL_THICKNESS_BILATERAL_TOLERANCE_MM - split,
        capture_v9.GROOVE_WIDTH_BILATERAL_TOLERANCE_MM - split,
        prior.maximum_axial_free_play_mm - required,
    ))
    payload = {
        "source_capture_v12_sha256": prior.evidence_sha256,
        "values": values,
