...
from .realized_waste_cartridge import (
    BLIND_KEY_DATUM_WORLD_MM, ROUTE_HANDOFF_WORLD_MM, box,
    build_realized_waste_cartridge, cylinder, volume,
)

SCHEMA = "MASCK_ONE_CELL11_DEVICE_CARTRIDGE_SERVICE_V1"
OWNER_PR = 140
...
def _annulus(start, length, od, bore):
    ...
    inner = cylinder((start[0] - .1, start[1], start[2]), (1, 0, 0), length + .2, bore)
...
def _shuttle():
    ...
    part = part.union(box((4, 6, 1.4), (33.5, -84, 21)))
    for cx, angle, retract in (
        (-26.8, math.degrees(math.atan2(4, 1.6)), 1.6),
        (26.8, 180 - math.degrees(math.atan2(4, 1.6)), 1.6),
        (-10.7, math.degrees(math.atan2(4, 1.4)), 1.4),
    ):
        slot = cq.Workplane("XY", origin=(cx, -84, 20)).slot2D(6.2, 1.6, angle).extrude(2)
        part = part.cut(slot)
...
def _receiver(tactile):
    ...
    fixed = fixed.union(box((12, 8, 2.6), (25, -82, -8.2)))
    fixed = fixed.union(box((4.2, .7, .6), (-38.4, -80.65, BLIND_KEY_DATUM_WORLD_MM[2])))
...
def _bolt_drive(bolt, sign):
    ...
    part = part.union(box((.9, .9, 2.8), (sign * 40.8, -82, 18.2)))
    part = part.union(box((14.8, .8, .6), (sign * 33.4, -82, 19.4)))
    part = part.union(cylinder((sign * 26, -82, 19.3), (0, 0, 1), 2.7, .8))
...
def _wet_drive():
    ...
    part = part.union(box((.8, 1, 2.2), (-40, -82, 15.4)))
    part = part.union(box((.8, .8, 4.2), (-40, -82, 17.3)))
    part = part.union(box((30, .7, .7), (-25, -82, 18.9)))
    part = part.union(cylinder((-10, -82, 18.7), (0, 0, 1), 3.3, .8))
    if not part.val().isValid() or len(part.val().Solids()) != 1:
...
class CartridgeDeviceService:
    ...
    def validate(self):
        _require_sources()
        for shape in (
            self.receiver, self.shuttle_locked, self.shuttle_service,
            self.left_locked, self.right_locked, self.left_service, self.right_service,
            self.wet_locked, self.wet_service, self.poppet_free,
            self.poppet_closed_service, self.poppet_open_locked, self.frame_mount_reference,
        ):
        ...
        for moving, fixed in (
            (self.shuttle_locked, self.receiver), (self.shuttle_service, self.receiver),
            (self.left_locked, self.receiver), (self.right_locked, self.receiver),
            (self.left_service, self.receiver), (self.right_service, self.receiver),
            (self.wet_locked, self.receiver), (self.wet_service, self.receiver),
            (self.left_locked, self.shuttle_locked), (self.right_locked, self.shuttle_locked),
            (self.wet_locked, self.shuttle_locked),
            (self.left_service, self.shuttle_service), (self.right_service, self.shuttle_service),
    ...
    def manifest(self):
        ...
        payload = {
            ...
            "kinematics": {
                "shuttle_axis_world": [0, 1, 0], "shuttle_travel_mm": SHUTTLE_TRAVEL_Y_MM,
            ...
            "wet_interface": {
                ...
                "leakage_validated": False, "closing_force_validated": False,
                "hydraulic_loss_validated": False, "backflow_performance_validated": False,
            },
            "digital_claims": {
        ...
        payload["manifest_sha256"] = sha256(json.dumps(payload, sort_keys=True, separators=(",", ":")).encode()).hexdigest()
        return payload
...
def build_cartridge_device_service():
    ...
    moving_cartridge = cq.Workplane(obj=cq.Compound.makeCompound([
    ...
    if volume(wet.intersect(moving_cartridge)) > TOL_MM3 or volume(wet_service.intersect(moving_cartridge)) > TOL_MM3:
        raise CartridgeDeviceServiceError("wet drive intersects cartridge material")

    poppet_free = cylinder((-39.4, -82, 14), (1, 0, 0), .35, POPPET_FREE_DIAMETER_MM)
    poppet_fit = cylinder((-39.4, -82, 14), (1, 0, 0), .35, POPPET_INSTALLED_DIAMETER_MM)
    ...
    frame_ref = cq.Workplane(obj=cq.Compound.makeCompound([
        box((10, 6, .2), (-25, -82, -9.55)).val(),
        box((10, 6, .2), (25, -82, -9.55)).val(),
    ]))
    result = CartridgeDeviceService(
        receiver, shuttle, shuttle.translate((0, SHUTTLE_TRAVEL_Y_MM, 0)),
        left, right, left_service, right_service, wet, wet_service, poppet_free,
        poppet_fit.translate((-WET_NOSE_RETRACTION_MM, 0, 0)),
        poppet_fit.translate((-.8, 0, 0)), frame_ref, corridor,
    )
