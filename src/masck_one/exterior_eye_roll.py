...
# A 3.0 mm edge radius on the curved crown needs finite posterior runout beyond the
# nominal radius. The former 0.20 mm reserve left only 3.20 mm total local depth and OCC
# could construct the feature but returned an invalid B-rep. Keep the extra material
# entirely wearer-side and inside the existing bounded hidden-support volume budget.
EYE_ROLL_SUPPORT_DEPTH_RESERVE_MM = 1.00
...
def _final_crown_face(authority: Authority, facial_reference: FacialReferenceLayer) -> cq.Face:
    ...
    constraints = inferior_turnover_constraints(authority, facial_reference, width, height, boundary_z)
    face = cq.Workplane("XY").interpPlate(boundary, constraints, thickness=0.0, combine=False, degree=3, nbPtsOnCur=20, nbIter=3).val()
...
def _posterior_eye_support_patch(face: cq.Face, *, wall_mm: float, roll_radius_mm: float, eye_width_mm: float, eye_height_mm: float, eye_x_mm: float, eye_y_mm: float, eye_cant_deg: float) -> cq.Shape:
    ...
    outer_support = _ellipse_cutter(eye_width_mm + 2.0 * EYE_ROLL_SUPPORT_BAND_MM, eye_height_mm + 2.0 * EYE_ROLL_SUPPORT_BAND_MM, eye_x_mm, eye_y_mm, angle_deg=eye_cant_deg).val()
    patch = posterior_delta.intersect(outer_support)
    return _single_valid(patch, "posterior eye-roll support patch")


def _rotated_ellipse_bbox_spans_mm(width_mm: float, height_mm: float, angle_deg: float) -> tuple[float, float]:
    theta = math.radians(angle_deg)
    cosine = math.cos(theta)
    sine = math.sin(theta)
    return math.sqrt((width_mm * cosine) ** 2 + (height_mm * sine) ** 2), math.sqrt((width_mm * sine) ** 2 + (height_mm * cosine) ** 2)


def _wearer_side_eye_edge(shape: cq.Shape, *, eye_width_mm: float, eye_height_mm: float, eye_x_mm: float, eye_y_mm: float, eye_cant_deg: float) -> cq.Edge:
    expected_x_span, expected_y_span = _rotated_ellipse_bbox_spans_mm(eye_width_mm, eye_height_mm, eye_cant_deg)
    ...
    for edge in shape.Edges():
        ...
        if abs(center_x - eye_x_mm) > EYE_EDGE_CENTER_TOLERANCE_MM or abs(center_y - eye_y_mm) > EYE_EDGE_CENTER_TOLERANCE_MM:
            continue
        if abs(float(bb.xlen) - expected_x_span) > EYE_EDGE_SPAN_TOLERANCE_MM or abs(float(bb.ylen) - expected_y_span) > EYE_EDGE_SPAN_TOLERANCE_MM:
            continue
        candidates.append(edge)
    if len(candidates) < 2:
        raise EyeInnerRollError(f"expected at least two exact rigid eye-opening edges near ({eye_x_mm}, {eye_y_mm}); found {len(candidates)}")
    ranked = sorted(candidates, key=lambda edge: 0.5 * (float(edge.BoundingBox().zmin) + float(edge.BoundingBox().zmax)))
    wearer = ranked[0]
    wearer_mean_z = 0.5 * (float(wearer.BoundingBox().zmin) + float(wearer.BoundingBox().zmax))
    anterior_mean_z = 0.5 * (float(ranked[-1].BoundingBox().zmin) + float(ranked[-1].BoundingBox().zmax))
    if wearer_mean_z >= anterior_mean_z:
        raise EyeInnerRollError("wearer-side eye edge is not posterior to rigid opening edge")
    return wearer


def _fillet_protected_eye_edges_independently(shape: cq.Shape, *, roll_radius_mm: float, eye_zones: tuple[object, object]) -> cq.Solid:
    rolled = _single_solid(shape, "protected eye-roll source")
    for index, zone in enumerate(eye_zones, start=1):
        edge = _wearer_side_eye_edge(rolled, eye_width_mm=zone.envelope_width_mm, eye_height_mm=zone.envelope_height_mm, eye_x_mm=zone.center.x, eye_y_mm=zone.center.y, eye_cant_deg=zone.angle_deg)
        try:
            feature = rolled.fillet(roll_radius_mm, [edge]).clean()
        except Exception as exc:
            raise EyeInnerRollError(f"protected eye {index} rigid-edge fillet failed") from exc
        rolled = _single_solid(feature, f"eye-rolled exterior after protected eye {index}")
    return rolled


def build_eye_rolled_exterior_shell(authority: Authority, facial_reference: FacialReferenceLayer, protected_volumes: ProtectedVolumeSet | None = None) -> cq.Workplane:
    protected = protected_volumes or build_current_protected_volumes(authority, facial_reference)
    base = build_inferior_turnover_exterior_shell(authority, facial_reference, protected).val()
    ...
    for zone in (left_zone, right_zone):
        patch = _posterior_eye_support_patch(face, wall_mm=wall, roll_radius_mm=roll_radius, eye_width_mm=zone.envelope_width_mm, eye_height_mm=zone.envelope_height_mm, eye_x_mm=zone.center.x, eye_y_mm=zone.center.y, eye_cant_deg=zone.angle_deg)
    ...
    pre_roll = _single_solid(shell.val(), "supported protected-clearance exterior")
    rolled = _fillet_protected_eye_edges_independently(pre_roll, roll_radius_mm=roll_radius, eye_zones=(left_zone, right_zone))
    ...
    if added_volume <= 0.0 or added_volume > EYE_ROLL_MAX_ADDED_VOLUME_MM3:
        raise EyeInnerRollError("eye-roll hidden support added volume is outside the bounded Cell 2 reserve")
...
def eye_inner_roll_manifest(authority: Authority) -> dict[str, object]:
    ...
    visual_width, visual_height = authority.pair("geometry", "eye", "visual_aperture_wh_mm")
    rigid_clearance = authority.number("geometry", "eye", "rigid_dynamic_keepout_clearance_mm")
    supported_depth = _supported_eye_depth_mm(wall, radius)
    return {
        "schema": SCHEMA,
        "radius_mm": radius,
        "visual_aperture_wh_mm": [visual_width, visual_height],
        "rigid_hard_envelope_wh_mm": [visual_width + 2.0 * rigid_clearance, visual_height + 2.0 * rigid_clearance],
        "rigid_dynamic_keepout_clearance_mm": rigid_clearance,
        "lateral_cant_deg": authority.number("geometry", "eye", "lateral_cant_deg"),
        ...
        "edge_selection_policy": "CANT_AWARE_RELEASED_RIGID_HARD_ENVELOPE_BBOX_THEN_POSTERIOR_Z",
        ...
        "visual_aperture_policy": "CONTROLLED_REFERENCE_REQUIRES_FUTURE_NONRIGID_VISIBLE_INTERFACE;NOT_REALIZED_AS_RIGID_MATERIAL",
        "visible_bezel_added": False,
        "external_a_surface_modified_by_support": False,
        "construction": "LOCAL_POSTERIOR_SUPPORT_PLUS_SAME_DOMAIN_CLEANUP_PLUS_SOLID_NORMALIZATION_PLUS_TWO_INDEPENDENT_CANT_AWARE_EXACT_WEARER_SIDE_RIGID_EDGE_FILLETS",
        "evidence_status": "DIGITAL_AUTHORITY_GEOMETRY_NOT_FIT_COMFORT_IMPACT_TOOLING_MATERIAL_OR_PHYSICAL_SAFETY_EVIDENCE",
    }
