...
PHASE_MIXED_WASTE="MIXED_AIR_LIQUID_FOAM_CONTAMINANT"; HYGIENE_WET_DRAINABLE="WET_DRAINABLE"
GEOMETRY_UNRESOLVED="GEOMETRY_UNRESOLVED_REQUIRES_REGISTERED_SKIN_FACING_SURFACE"; BUFFER_UNRESOLVED="TRANSIENT_CAPACITY_UNRESOLVED_REQUIRES_MIXED_PHASE_BENCH_EVIDENCE"
ROUTE_DESTINATION="WASTE_PUMP_INLET_ITERATION_26_INTERFACE"; EVIDENCE_STATUS="DIGITAL_WASTE_ACQUISITION_TOPOLOGY_ONLY_NOT_RECOVERY_RESIDUAL_LEAKAGE_HYGIENE_OR_PHYSICAL_EVIDENCE"
REGIONS=("FOREHEAD","LEFT_CHEEK","RIGHT_CHEEK","NOSE_T_ZONE","CHIN_PERIORAL"); _SHA_RE=re.compile(r"[0-9a-f]{64}\Z")
def _exact(v,e,l):
    if type(v) is not str or v!=e: raise WasteAcquisitionError(f"{l} must use its controlled exact state")
def _sha(v,l):
    if type(v) is not str or _SHA_RE.fullmatch(v) is None: raise WasteAcquisitionError(f"{l} must be canonical lowercase SHA-256")
...
class WasteRegionIntent:
    ...
    def __post_init__(self):
        ...
        _exact(self.phase_semantics,PHASE_MIXED_WASTE,"waste phase semantics"); _exact(self.hygiene_class,HYGIENE_WET_DRAINABLE,"waste hygiene class"); _exact(self.gutter_geometry_status,GEOMETRY_UNRESOLVED,"gutter geometry status"); _exact(self.capillary_geometry_status,GEOMETRY_UNRESOLVED,"capillary geometry status"); _exact(self.transient_buffer_status,BUFFER_UNRESOLVED,"transient buffer status"); _exact(self.destination,ROUTE_DESTINATION,"waste destination")
    ...
    def manifest(self): return {n:getattr(self,n) for n in self.__dataclass_fields__}
@dataclass(frozen=True,slots=True)
class WasteAcquisitionArchitecture:
    ...
    def __post_init__(self):
        ...
        if type(self.recovery_ratio_min) not in (int,float) or not 0<float(self.recovery_ratio_min)<=1: raise WasteAcquisitionError("recovery ratio must be a bounded authority value")
        if type(self.residual_free_liquid_max_uL) not in (int,float) or float(self.residual_free_liquid_max_uL)<0: raise WasteAcquisitionError("residual free-liquid limit must be non-negative")
    ...
    @property
    def architecture_sha256(self): return _digest(self.manifest())
def build_waste_acquisition_architecture(authority:Authority,distribution:DistributionGeometryArchitecture)->WasteAcquisitionArchitecture:
    ...
    return WasteAcquisitionArchitecture(distribution.architecture_sha256,authority.get("project","authority_revision"),float(recovery),float(residual),regions,False,EVIDENCE_STATUS)
