...
def export_release(output_dir: str | Path = "generated", model: MasckOneModel | None = None) -> dict:
    ...
    # attachment that does not yet exist. Prompt 10 hair/pinch solids are reference
    # keepouts only and likewise must never enter the physical compound as material.
    ...
    report = {
        ...
        "note": (
            ...
            "unworn/unpowered index-pin service sequence. Prompt 10 adds standalone hazard/reference geometry for guide "
            "nip regions, pin/clip paths, future root-capture interfaces, scalp-side hair approach and a candidate-only "
            "right-latch hazard/access overlay. These keepout solids are not physical guards and are not inserted into "
            "the development assembly. Neither occipital source nor adjustment successor is inserted until a frame-side "
            "positive-capture / housing load path is realized. No anthropometric fit, comfort, preload, contact pressure, "
            "hair entrapment prevention, pinch-force safety, structural capacity, fatigue, wear, jam resistance, wet "
            "one-hand release performance or physical retention performance is established. Crown support remains a "
            "separate reserved interface corridor. Digital topology/manifests and analysis frameworks are not physical "
            "validation evidence."
        ),
    }
