...
class ViewDelta:
    ...
    def __post_init__(self) -> None:
        ...
        values = (
            self.horizontal_span_delta_mm,
            self.vertical_span_delta_mm,
            self.horizontal_span_delta_fraction,
            self.vertical_span_delta_fraction,
            self.aspect_ratio_delta,
            self.centroid_horizontal_shift_mm,
            self.centroid_vertical_shift_mm,
        )
    ...
    def manifest(self) -> dict[str, object]:
        return {
            ...
            "horizontal_span_delta_mm": self.horizontal_span_delta_mm,
            "vertical_span_delta_mm": self.vertical_span_delta_mm,
            "horizontal_span_delta_fraction": self.horizontal_span_delta_fraction,
            "vertical_span_delta_fraction": self.vertical_span_delta_fraction,
            "aspect_ratio_delta": self.aspect_ratio_delta,
            "centroid_horizontal_shift_mm": self.centroid_horizontal_shift_mm,
...
class VisualRegressionComparison:
    ...
    evidence_status: str = "DIGITAL_GEOMETRY_REGRESSION_METRICS_ONLY_NO_AESTHETIC_OR_PHYSICAL_ACCEPTANCE_THRESHOLD"
    ...
    def __post_init__(self) -> None:
        identities = (
            ...
            self.baseline_sample_manifest_sha256,
            self.candidate_sample_manifest_sha256,
        )
        ...
        if not isinstance(self.evidence_status, str) or not self.evidence_status.strip():
            raise VisualRegressionError("Regression evidence status must be explicit nonblank text")
    ...
    def manifest(self, *, include_sha: bool = True) -> dict[str, object]:
        payload: dict[str, object] = {
            ...
            "baseline_sample_manifest_sha256": self.baseline_sample_manifest_sha256,
            "candidate_sample_manifest_sha256": self.candidate_sample_manifest_sha256,
            "view_deltas": [delta.manifest() for delta in self.view_deltas],
...
def compare_visual_reports(
    baseline: VisualInspectionReport,
    candidate: VisualInspectionReport,
    ...
    for base_view, candidate_view in zip(baseline.views, candidate.views, strict=True):
        ...
        deltas.append(
            ViewDelta(
                ...
                horizontal_span_delta_mm=h_delta,
                vertical_span_delta_mm=v_delta,
                horizontal_span_delta_fraction=h_delta / base_view.horizontal_span_mm,
                vertical_span_delta_fraction=v_delta / base_view.vertical_span_mm,
                aspect_ratio_delta=candidate_view.aspect_ratio - base_view.aspect_ratio,
                centroid_horizontal_shift_mm=candidate_view.centroid_horizontal_mm - base_view.centroid_horizontal_mm,
                centroid_vertical_shift_mm=candidate_view.centroid_vertical_mm - base_view.centroid_vertical_mm,
            )
        )

    return VisualRegressionComparison(
        baseline_report_sha256=baseline.report_sha256,
        candidate_report_sha256=candidate.report_sha256,
        baseline_sample_manifest_sha256=baseline.source_sample_manifest_sha256,
        candidate_sample_manifest_sha256=candidate.source_sample_manifest_sha256,
        view_deltas=tuple(deltas),
    )
