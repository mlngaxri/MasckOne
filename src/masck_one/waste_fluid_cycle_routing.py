...
class CycleResolvedRoutingClosure:
    ...
    def __post_init__(self) -> None:
        """Reject structurally inconsistent evidence, including manually forged closures."""
        if type(self.cycles) is not tuple or not self.cycles:
            raise WasteFluidAccountingError("cycle-resolved routing closure requires a nonempty tuple of cycles")
        if type(self.service) is not ServiceRoutingClosure:
            raise WasteFluidAccountingError("cycle-resolved routing closure requires exact ServiceRoutingClosure evidence")
        if any(type(state) is not CycleRoutingScreen for state in self.cycles):
            raise WasteFluidAccountingError("cycle-resolved routing closure requires exact CycleRoutingScreen evidence")
        expected_cycles = tuple(range(1, len(self.cycles) + 1))
        if tuple(state.cycle for state in self.cycles) != expected_cycles:
            raise WasteFluidAccountingError("cycle-resolved routing evidence must be contiguous and one-indexed")
        if self.service.cycles != len(self.cycles):
...
def _assert_cycle_service_parity(cycle_closure: CycleResolvedRoutingClosure) -> None:
    ...
    Nominal and prime cartridge routing are reconciled independently. Checking only
    their sum would allow equal-and-opposite component corruption to preserve the
    aggregate while changing the engineering meaning of the evidence.
    """
