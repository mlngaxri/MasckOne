...
class StructuralFrameCarrierPreloadArchitecture:
    ...
    def __post_init__(self) -> None:
        if len(self.source_interface_architecture_sha256) != 64:
            raise StructuralFrameCarrierPreloadError("source interface identity must be SHA-256")
...
def _feature_for_interface(interface) -> tuple[cq.Workplane, cq.Workplane]:
    ...
    ).translate((root_x, cy, z_top - LEAF_ROOT_HEIGHT_MM))
    leaf_x = cx + sign * LEAF_OUTBOARD_OFFSET_MM
    leaf = cq.Workplane("XY").box(
        LEAF_WIDTH_MM,
        LEAF_LENGTH_MM,
        LEAF_HEIGHT_MM,
        centered=(True, True, False),
    ).translate((leaf_x, cy, z_top - LEAF_ROOT_HEIGHT_MM))
    feature = root.union(leaf)
    counterface_x = leaf_x + sign * (LEAF_WIDTH_MM / 2.0 + NOMINAL_LATERAL_GAP_MM + COUNTERFACE_WIDTH_MM / 2.0)
    counterface = cq.Workplane("XY").box(
        COUNTERFACE_WIDTH_MM,
        COUNTERFACE_LENGTH_MM,
        COUNTERFACE_HEIGHT_MM,
        centered=(True, True, False),
    ).translate((counterface_x, cy, z_top - LEAF_ROOT_HEIGHT_MM))
...
def build_structural_frame_carrier_preload(*, interfaces: StructuralFrameCarrierInterfaceArchitecture | None = None) -> StructuralFrameCarrierPreloadArchitecture:
    ...
    for source in interfaces.interfaces:
        ...
        built.append(CarrierPreloadFeature(
            source.reaction_id,
            source.center_xy_mm,
            feature,
            round(capture, 8),
...
def export_structural_frame_carrier_preload(output_dir: Path) -> dict[str, object]:
    ...
    (output_dir / "structural_frame_carrier_preload_manifest.json").write_text(json.dumps(manifest, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    return manifest
