...
CONTACT_CONNECTIVITY_STATUS = "ONE_EDGE_CONNECTED_CONTACT_FIELD_ON_REFINED_PLANAR_DEVELOPMENT_MESH"
MATERIAL_CONTINUITY_STATUS = "PHYSICAL_MATERIAL_CONTINUITY_UNRESOLVED_UNTIL_DETAILED_MEMBRANE_AND_ATTACHMENT_CAD"
...
class CompliantInterfaceTopology:
    ...
    def __post_init__(self) -> None:
        ...
        if self.anatomical_validation_eligible:
            raise InterfaceTopologyError("Iteration-10 topology cannot be promoted to validation evidence")
        zone_ids = [zone.zone_id for zone in self.zones]
        if not zone_ids or len(zone_ids) != len(set(zone_ids)):
            raise InterfaceTopologyError("Interface parameter-zone IDs must be non-empty and unique")
        ...
        unknown = {assignment.parameter_zone_id for assignment in self.assignments} - set(zone_ids)
        if unknown:
    ...
    def contact_components(self, coverage: FacialCoverageMesh) -> tuple[ContactComponent, ...]:
        ...
        raw_components.sort(
            key=lambda component: (-sum(assignment_by_id[i].area_mm2 for i in component), min(component))
        )
        ...
        for component_index, component in enumerate(raw_components):
            ...
            summaries.append(
                ContactComponent(
                    ...
                    parameter_zone_ids=tuple(sorted({assignment_by_id[i].parameter_zone_id for i in indices})),
                    area_mm2=sum(assignment_by_id[i].area_mm2 for i in indices),
    ...
    def topology_sha256(self) -> str:
        payload = {
            ...
            "assignments": [
                [item.triangle_index, item.coverage_region_id, item.parameter_zone_id,
                 round(item.area_mm2, 12), item.contact_intent, item.protected_opening]
                for item in self.assignments
            ],
...
def _build_parameter_zones() -> tuple[InterfaceParameterZone, ...]:
    ...
    return (
        InterfaceParameterZone(ZONE_GENERAL_FACE, "primary compliant skin-contact cleansing field",
                               (REGION_ACTIVE_OTHER,), thickness_status="UNRESOLVED_PENDING_INTERFACE_GEOMETRY_AND_MATERIAL_SELECTION", **contact_common),
        InterfaceParameterZone(ZONE_T_FOREHEAD, "forehead T-zone skin-contact cleansing field",
                               (REGION_T_FOREHEAD,), thickness_status="UNRESOLVED_PENDING_INTERFACE_GEOMETRY_AND_MATERIAL_SELECTION", **contact_common),
        InterfaceParameterZone(ZONE_T_NOSE_PHILTRUM, "central T-zone and nose-to-upper-lip cleansing target field",
                               (REGION_T_NOSE_PHILTRUM,), thickness_status="DEDICATED_NASAL_SUBSYSTEM_BOUNDARY_REQUIRED_BEFORE_THICKNESS_APPLICATION", **contact_common),
        InterfaceParameterZone(ZONE_OPENING_EYE_LEFT, "left eye protected opening", (REGION_PROTECTED_EYE_LEFT,), **opening_common),
        InterfaceParameterZone(ZONE_OPENING_EYE_RIGHT, "right eye protected opening", (REGION_PROTECTED_EYE_RIGHT,), **opening_common),
        InterfaceParameterZone(ZONE_OPENING_MOUTH, "mouth protected opening", (REGION_PROTECTED_MOUTH,), **opening_common),
        InterfaceParameterZone(ZONE_OPENING_NOSTRIL_LEFT, "left nostril/airway protected opening", (REGION_PROTECTED_NOSTRIL_LEFT,), **opening_common),
        InterfaceParameterZone(ZONE_OPENING_NOSTRIL_RIGHT, "right nostril/airway protected opening", (REGION_PROTECTED_NOSTRIL_RIGHT,), **opening_common),
    )


def build_compliant_interface_topology(authority: Authority, coverage: FacialCoverageMesh) -> CompliantInterfaceTopology:
    ...
    if len(components) != 1:
        diagnostic = [component.manifest() for component in components]
        raise InterfaceTopologyError(
            "Refined development contact topology must remain one edge-connected field; "
            f"found {len(components)} components: {diagnostic}"
        )
    return topology