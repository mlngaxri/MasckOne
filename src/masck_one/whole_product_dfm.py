...
"""Released-main manufacturing part architecture for Masck One.

The contract defines part boundaries, assembly order, service ownership, hygiene
classification intent and prototype/production process intent. It creates no product
B-rep and does not promote unmerged specialist work or physical validation.
"""
...
class ManufacturingRules:
    ...
    def __post_init__(self) -> None:
        ...
        tolerance = _real(self.visible_seam_tolerance_mm, label="visible seam tolerance", positive=True)
        ...
        object.__setattr__(self, "visible_seam_tolerance_mm", tolerance)
...
class PartFamily:
    ...
    def __post_init__(self) -> None:
        ...
        for label, value in (
            ("part display name", self.display_name),
            ("source reference", self.source_ref),
            ("hygiene provenance", self.hygiene_provenance),
            ("closure semantics", self.closure_semantics),
        ):
...
class CandidateBinding:
    ...
    def __post_init__(self) -> None:
        ...
        if type(self.affected_part_ids) is not tuple or not self.affected_part_ids:
            raise DfmArchitectureError("candidate affected part IDs must be a nonempty tuple")
...
class WholeProductDfmArchitecture:
    ...
    def __post_init__(self) -> None:
        ...
        if type(self.rules) is not ManufacturingRules or type(self.parts) is not tuple or not self.parts:
            raise DfmArchitectureError("DFM architecture requires immutable controlled rules and parts")
        if type(self.observed_candidates) is not tuple:
            raise DfmArchitectureError("candidate observations must be an immutable tuple")
    ...
    def _validate_graph_and_coverage(self) -> None:
        ...
        for candidate in self.observed_candidates:
            ...
            if candidate.pr_number in seen_prs:
                raise DfmArchitectureError("candidate PR bindings cannot repeat")
            seen_prs.add(candidate.pr_number)
    ...
    def manifest(self, *, include_sha: bool = True) -> dict[str, object]:
        payload: dict[str, object] = {
            ...
            "system_summary": {
                system: {
                    "part_family_count": sum(part.system == system for part in self.parts),
                    "physical_part_quantity": sum(part.quantity for part in self.parts if part.system == system),
                    "unresolved_part_family_count": sum(
                        part.system == system and part.maturity == MATURITY_UNRESOLVED_REQUIRED
                        for part in self.parts
                    ),
                }
        ...
        if include_sha:
            payload["dfm_architecture_sha256"] = sha256(
...
def _build_parts() -> tuple[PartFamily, ...]:
    parts = (
        ...
        # Frame, retention and quick release
        ...
        # Fresh-water serviceable module
        _p("WATER-RESERVOIR-BODY", "Fresh-water reservoir body", SYSTEM_FRESH_FLUID, OWNER_CELL_4, 1, ROLE_RESERVOIR, MATURITY_UNRESOLVED_REQUIRED, "released-envelope-plus-unmerged-reservoir-candidate", H_WET_REMOVABLE, SERVICE_USER_REMOVABLE, PATH_REQUIRED, PROTOTYPE_PRINT_MACHINE, PROCESS_RIGID_POLYMER_TBD, 50, ("MASCK_ONE-DFM-REACTION-FRAME",), hygiene_provenance="CURRENT_WATER_ARCHITECTURE_EXPLICIT_WET_REMOVABLE"),
        _p("WATER-RESERVOIR-LID-SEAL", "Fresh-water reservoir lid seal", SYSTEM_FRESH_FLUID, OWNER_CELL_4, 1, ROLE_SEAL, MATURITY_UNRESOLVED_REQUIRED, "seal-groove-land-candidate-no-seal-material", H_WET_REMOVABLE, SERVICE_USER_REMOVABLE, PATH_REQUIRED, PROTOTYPE_SOFT_TOOL, PROCESS_SEAL_TBD, 52, ("MASCK_ONE-DFM-WATER-RESERVOIR-BODY",), "CONTROLLED_SEAL_LAND_AND_COMPRESSION_GEOMETRY_REQUIRED", "CURRENT_WATER_ARCHITECTURE_REQUIRES_SEAL_WET_REMOVABLE_CLASS_PROVISIONAL"),
        _p("WATER-PICKUP-CONNECTOR", "Fresh-water pickup connector or fitting", SYSTEM_FRESH_FLUID, OWNER_CELL_4, 1, ROLE_CONNECTOR, MATURITY_UNRESOLVED_REQUIRED, "pickup-connector-reservation-no-fitting-selected", H_WET_REMOVABLE, SERVICE_TECHNICIAN_REMOVABLE, PATH_REQUIRED, PROTOTYPE_PURCHASED, PROCESS_PURCHASED_REFERENCE, 53, ("MASCK_ONE-DFM-WATER-RESERVOIR-BODY",), "POSITIVE_RETAINED_PICKUP_CONNECTION_AND_SEAL_INTERFACE_REQUIRED"),
        _p("WATER-RESERVOIR-LID", "Fresh-water captured sliding lid", SYSTEM_FRESH_FLUID, OWNER_CELL_4, 1, ROLE_COVER, MATURITY_UNRESOLVED_REQUIRED, "unmerged-captured-slide-lid-candidate", H_WET_REMOVABLE, SERVICE_USER_REMOVABLE, PATH_REQUIRED, PROTOTYPE_PRINT_MACHINE, PROCESS_RIGID_POLYMER_TBD, 54, ("MASCK_ONE-DFM-WATER-RESERVOIR-BODY", "MASCK_ONE-DFM-WATER-RESERVOIR-LID-SEAL"), "CAPTURE_RAIL_GEOMETRY_CANDIDATE_REQUIRES_RETAINED_KEY"),
        _p("WATER-LID-RETENTION-KEY", "Fresh-water lid transverse retention key", SYSTEM_FRESH_FLUID, OWNER_CELL_4, 1, ROLE_LATCH, MATURITY_UNRESOLVED_REQUIRED, "unmerged-water-reservoir-closure-key-candidate", H_WET_REMOVABLE, SERVICE_USER_REMOVABLE, PATH_REQUIRED, PROTOTYPE_PRINT_MACHINE, PROCESS_STRUCTURAL_CARRIER_TBD, 55, ("MASCK_ONE-DFM-WATER-RESERVOIR-LID",), "POSITIVE_KEY_RETENTION_CANDIDATE_FORCE_STRAIN_AND_WET_USE_UNVALIDATED"),
        _p("WATER-FILL-SEAL", "Fresh-water fill closure seal", SYSTEM_FRESH_FLUID, OWNER_CELL_4, 1, ROLE_SEAL, MATURITY_UNRESOLVED_REQUIRED, "fill-closure-reservation-no-seal-selected", H_WET_REMOVABLE, SERVICE_USER_REMOVABLE, PATH_REQUIRED, PROTOTYPE_SOFT_TOOL, PROCESS_SEAL_TBD, 56, ("MASCK_ONE-DFM-WATER-RESERVOIR-LID",), "CONTROLLED_FILL_SEAL_LAND_AND_COMPRESSION_REQUIRED"),
        _p("WATER-VENT-BARRIER", "Fresh-water vent liquid-barrier element", SYSTEM_FRESH_FLUID, OWNER_CELL_4, 1, ROLE_SEAL, MATURITY_UNRESOLVED_REQUIRED, "vent-barrier-reservation-no-hardware-selected", H_WET_REMOVABLE, SERVICE_TECHNICIAN_REMOVABLE, PATH_REQUIRED, PROTOTYPE_PURCHASED, PROCESS_PURCHASED_REFERENCE, 56, ("MASCK_ONE-DFM-WATER-RESERVOIR-LID",), "VENT_BARRIER_RETENTION_AND_SEAL_INTERFACE_REQUIRED"),
        _p("WATER-FILL-CLOSURE", "Fresh-water user fill closure", SYSTEM_FRESH_FLUID, OWNER_CELL_4, 1, ROLE_COVER, MATURITY_UNRESOLVED_REQUIRED, "fill-closure-reservation-no-hardware-selected", H_WET_REMOVABLE, SERVICE_USER_REMOVABLE, PATH_REQUIRED, PROTOTYPE_PRINT_MACHINE, PROCESS_RIGID_POLYMER_TBD, 57, ("MASCK_ONE-DFM-WATER-FILL-SEAL",), "POSITIVE_USER_RETAINED_FILL_CLOSURE_REQUIRED"),

        # Cleanser, manifold and fresh routes
        ...
        # Waste system
        _p("WASTE-BACKBONE-ROUTE-SET", "Mixed-waste backbone route set", SYSTEM_WASTE, OWNER_CELL_4, 1, ROLE_ROUTE, MATURITY_RELEASED_TOPOLOGY, "src/masck_one/realized_waste_backbone.py:centerline-geometry", H_WET_DRAINABLE, SERVICE_TECHNICIAN_REMOVABLE, PATH_REQUIRED, PROTOTYPE_FLEXIBLE_ROUTE, PROCESS_FLEXIBLE_ROUTE_TBD, 62, ("MASCK_ONE-DFM-ROUTE-CARRIER-CLIP-SET",)),
        ...
        _p("BATTERY-CARRIER", "Non-compressive battery carrier", SYSTEM_ELECTRICAL_HMI, OWNER_CELL_4, 1, ROLE_CARRIER, MATURITY_UNRESOLVED_REQUIRED, "portable-donor-PR64-must-be-rebound", H_SEALED, SERVICE_TECHNICIAN_REMOVABLE, PATH_REQUIRED, PROTOTYPE_PRINT_MACHINE, PROCESS_STRUCTURAL_CARRIER_TBD, 32, ("MASCK_ONE-DFM-REACTION-FRAME",)),
        ...
        _p("PCB-CARRIER", "Control and power PCB carrier", SYSTEM_ELECTRICAL_HMI, OWNER_CELL_4, 1, ROLE_CARRIER, MATURITY_UNRESOLVED_REQUIRED, "portable-donor-PR64-must-be-rebound", H_SEALED, SERVICE_TECHNICIAN_REMOVABLE, PATH_REQUIRED, PROTOTYPE_PRINT_MACHINE, PROCESS_STRUCTURAL_CARRIER_TBD, 36, ("MASCK_ONE-DFM-DRY-BAY-HOUSING",)),
...
def build_whole_product_dfm_architecture(
    ...
    *,
    ...
    rules = ManufacturingRules(
        manufacturing.get("mold_draft_nominal_deg"),
        rib_range[0],
        rib_range[1],
        seam.get("gap_mm"),
        seam.get("tolerance_mm"),
        seam.get("flush_mismatch_max_mm"),
        tuple(hygiene),
    )
