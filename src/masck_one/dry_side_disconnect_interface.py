...
SOURCE_MAIN_SHA = "d5af8b976da412d4cdc7658841e7bb2021723b24"
...
def _geometry(shape):
    ...
    return {"bounds_world_mm": [float(bb.xmin), float(bb.xmax), float(bb.ymin), float(bb.ymax), float(bb.zmin), float(bb.zmax)], "volume_mm3": volume}
...
class BatteryDisconnectInterface:
    ...
    def manifest(self):
        self.validate()
        payload = {"schema": SCHEMA, "source_main_sha": SOURCE_MAIN_SHA, "source_donor_sha": SOURCE_DONOR_SHA, "coordinate_frame_id": WORLD_FRAME_ID, "mating_datum_world_mm": list(MATING_DATUM_WORLD_MM), "mating_axis_world": list(MATING_AXIS_WORLD), "disconnect_travel_mm": DISCONNECT_TRAVEL_MM, "geometry": {"connector_reservation": _geometry(self.connector_reservation), "key_reservation": _geometry(self.key_reservation), "strain_relief_reservation": _geometry(self.strain_relief_reservation), "disconnect_service_sweep": _geometry(self.disconnect_service_sweep)}, "interface_status": "GEOMETRIC_MATING_AND_SERVICE_DATUM_REALIZED_CONNECTOR_UNSELECTED", "electrical_ratings_selected": False, "connector_selected": False, "retention_force_validated": False, "ingress_validated": False, "physical_service_validated": False, "evidence_status": "DIGITAL_PACKAGING_AND_SERVICE_GEOMETRY_ONLY"}
...
def build_battery_disconnect_interface():
    ...
    connector = _box(CONNECTOR_RESERVATION_MM, (x, y + CONNECTOR_RESERVATION_MM[1] / 2.0, z))
    key = _box(KEY_RIB_MM, (x + CONNECTOR_RESERVATION_MM[0] / 2.0 - KEY_RIB_MM[0] / 2.0, y + 2.0, z + CONNECTOR_RESERVATION_MM[2] / 2.0 + KEY_RIB_MM[2] / 2.0))
    strain = _box(STRAIN_RELIEF_RESERVATION_MM, (x, y + CONNECTOR_RESERVATION_MM[1] + STRAIN_RELIEF_RESERVATION_MM[1] / 2.0, z))
    sweep = _box((CONNECTOR_RESERVATION_MM[0], CONNECTOR_RESERVATION_MM[1] + DISCONNECT_TRAVEL_MM, CONNECTOR_RESERVATION_MM[2]), (x, y + (CONNECTOR_RESERVATION_MM[1] + DISCONNECT_TRAVEL_MM) / 2.0, z))
