...
class ThermalCommand:
    ...
    def __post_init__(self) -> None:
        ...
        for name, value in (("warm_enable", self.warm_enable), ("cool_enable", self.cool_enable), ("inhibited", self.inhibited)):
        ...
        expected_enables = {ThermalMode.OFF: (False, False), ThermalMode.WARM: (True, False), ThermalMode.COOL: (False, True)}
        if (self.warm_enable, self.cool_enable) != expected_enables[self.mode]:
            raise ThermalControlError("mode and thermal output enables describe an impossible command")
        if self.inhibited:
    ...
    def to_wire(self) -> ThermalCommandWire:
        ...
        return {"mode": self.mode.value, "warm_enable": self.warm_enable, "cool_enable": self.cool_enable, "inhibited": self.inhibited, "reason": self.reason.value if self.reason is not None else None}
    ...
    def from_wire(cls, payload: object) -> ThermalCommand:
        ...
        if keys != required:
            ...
            raise ThermalControlError(f"thermal wire payload fields mismatch: missing={missing}, extra={extra}")
        ...
        if reason_value is None:
            ...
            try:
                ...
                raise ThermalControlError("wire reason is not a recognized ThermalInhibitReason") from exc
        return cls(mode=mode, warm_enable=payload["warm_enable"], cool_enable=payload["cool_enable"], inhibited=payload["inhibited"], reason=reason)


class ThermalCommandInterlock:
    """Fail-closed WARM/COOL command arbiter with explicit recovery gating."""

    def command(self, *, warm_requested: bool, cool_requested: bool, recovery_complete: bool) -> ThermalCommand:
        for name, value in (("warm_requested", warm_requested), ("cool_requested", cool_requested), ("recovery_complete", recovery_complete)):
            if type(value) is not bool:
                raise ThermalControlError(f"{name} must be an exact bool")
        if warm_requested and cool_requested:
            return ThermalCommand(ThermalMode.OFF, False, False, inhibited=True, reason=ThermalInhibitReason.CONFLICTING_REQUESTS)
        if cool_requested and not recovery_complete:
            return ThermalCommand(ThermalMode.OFF, False, False, inhibited=True, reason=ThermalInhibitReason.RECOVERY_INCOMPLETE)
        if warm_requested:
