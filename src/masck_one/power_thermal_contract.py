...
class PowerLoadWindow:
    ...
    def __post_init__(self) -> None:
        ...
        if self.status is LoadStatus.UNRESOLVED:
            if any(value is not None for value in values) or evidence:
                raise PowerThermalContractError("UNRESOLVED power load cannot carry numeric inputs or evidence")
            return

        if any(value is None for value in values):
            raise PowerThermalContractError("bounded/measured power load requires complete power and duration bounds")
        min_power = _real(self.minimum_power_W, label=f"{self.load_id} minimum power", nonnegative=True)
        max_power = _real(self.maximum_power_W, label=f"{self.load_id} maximum power", nonnegative=True)
        min_duration = _real(self.minimum_duration_s, label=f"{self.load_id} minimum duration", nonnegative=True)
        max_duration = _real(self.maximum_duration_s, label=f"{self.load_id} maximum duration", nonnegative=True)
        ...
        if not evidence:
            raise PowerThermalContractError("bounded/measured power load requires explicit evidence provenance")
...
class PowerBudget:
    ...
    def __post_init__(self) -> None:
        ...
        if any(type(item) is not PowerLoadWindow for item in loads):
            raise PowerThermalContractError("power loads must contain exact PowerLoadWindow records")
        ids = tuple(item.load_id for item in loads)
        if ids != LOAD_IDS:
            raise PowerThermalContractError("power load ledger must contain the complete controlled load set in canonical order")
    ...
    def energy_bounds_Wh(self) -> tuple[float, float]:
        if not self.complete_for_energy_bounds:
            raise PowerThermalContractError("power budget cannot close while any load remains unresolved")
...
class ThermalRiskGate:
    ...
    def __post_init__(self) -> None:
        ...
        if self.status is ThermalStatus.BLOCKED:
            if self.model_bounds is not None or self.unit is not None or evidence:
                raise PowerThermalContractError("BLOCKED thermal gate cannot carry model bounds or closure evidence")
            return

        bounds = _exact_tuple(self.model_bounds, label=f"{self.risk_id} model bounds")
        if len(bounds) != 2:
            raise PowerThermalContractError("thermal model bounds must contain exactly two values")
        ...
        if type(self.unit) is not str or not self.unit or self.unit.strip() != self.unit:
            raise PowerThermalContractError("bounded thermal gate requires an exact nonblank unit")
        if not evidence:
            raise PowerThermalContractError("bounded/measured thermal gate requires evidence provenance")
...
class ChargingArchitectureBoundary:
    ...
    def __post_init__(self) -> None:
        if type(self.convention) is not str or self.convention != CHARGING_PREFERENCE:
            raise PowerThermalContractError("charging convention must remain the controlled commercial preference")
        if type(self.status) is not ChargingConventionStatus or self.status is not CHARGING_PREFERENCE_STATUS:
            raise PowerThermalContractError("charging convention is not yet architecture-selected or verified")
        for label, value in (
        ...
        if self.active_wet_cycle_charging_allowed:
            raise PowerThermalContractError("charging during an active wet cycle is not authorized by current evidence")
        if self.app_required_for_charge_recovery:
            raise PowerThermalContractError("core charge fault recovery cannot be app-only in the current architecture")
        if self.ingress_architecture_verified or self.electrical_protection_verified:
            raise PowerThermalContractError("charging ingress/protection cannot be promoted before physical/electrical evidence")
...
class PowerThermalEvidenceContract:
    ...
    def __post_init__(self) -> None:
        ...
        if type(self.battery) is not BatteryPackagingBenchmark:
            raise PowerThermalContractError("battery benchmark must use the exact controlled type")
        if type(self.power_budget) is not PowerBudget:
            raise PowerThermalContractError("power budget must use the exact controlled type")
        gates = _exact_tuple(self.thermal_gates, label="thermal gates")
        if any(type(item) is not ThermalRiskGate for item in gates):
            raise PowerThermalContractError("thermal gates must contain exact ThermalRiskGate records")
        ids = tuple(item.risk_id for item in gates)
        if ids != THERMAL_RISK_IDS:
            raise PowerThermalContractError("thermal gate ledger must contain the complete controlled risk set in canonical order")
        if type(self.charging) is not ChargingArchitectureBoundary:
            raise PowerThermalContractError("charging boundary must use the exact controlled type")
        if type(self.physical_validation_eligible) is not bool or self.physical_validation_eligible:
            raise PowerThermalContractError("digital power/thermal contract is not physical validation evidence")
        object.__setattr__(self, "thermal_gates", tuple(gates))

    def validate_current_authority(self, authority: Authority) -> None:
        if type(authority) is not Authority:
            raise PowerThermalContractError("authority must use the exact controlled Authority type")
        revision = authority.get("project", "authority_revision")
        if type(revision) is not str or revision != self.authority_revision:
            raise PowerThermalContractError("power/thermal contract is stale for current authority revision")
        raw = authority.get("battery_reference")
        if type(raw) is not dict:
            raise PowerThermalContractError("battery reference authority must be an exact mapping")
        expected = BatteryPackagingBenchmark(
            candidate=raw.get("candidate"),
            nominal_voltage_V=raw.get("nominal_voltage_V"),
            capacity_mAh=raw.get("capacity_mAh"),
            envelope_mm=tuple(raw.get("envelope_mm")) if type(raw.get("envelope_mm")) is list else raw.get("envelope_mm"),
            mass_g=raw.get("mass_g"),
            status=raw.get("status"),
        )
        if self.battery != expected:
            raise PowerThermalContractError("battery packaging benchmark is stale for current authority")
...
def build_cell4_power_thermal_contract(authority: Authority) -> PowerThermalEvidenceContract:
    ...
    if type(envelope) is not list or len(envelope) != 3:
        raise PowerThermalContractError("battery authority envelope must be an exact three-item list")
    battery = BatteryPackagingBenchmark(
        candidate=raw.get("candidate"),
        nominal_voltage_V=raw.get("nominal_voltage_V"),
        capacity_mAh=raw.get("capacity_mAh"),
    ...
    power_budget = PowerBudget(
        loads=tuple(PowerLoadWindow(load_id, LoadStatus.UNRESOLVED) for load_id in LOAD_IDS),
