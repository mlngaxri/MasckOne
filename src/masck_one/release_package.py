...
def publish_package(output_dir: str | Path, writer: Callable[[Path], None]) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="masck-one-export-", dir=output.parent) as tmp:
        ...
        # A previous, unlisted STEP could otherwise be mistaken for current CAD.
...
def verify_package(output_dir: str | Path) -> dict:
    ...
    if manifest.get("schema") != PACKAGE_SCHEMA or manifest.get("scope") != "DEVELOPMENT_ONLY":
        raise ExportValidationError("Unknown package schema or scope")
    ...
    for name, expected in files.items():
        if (not isinstance(name, str) or not name or name in (".", "..", MANIFEST_NAME)
            ...
            raise ExportValidationError("Package filenames must be local basenames")
        path = output / name
