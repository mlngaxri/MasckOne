...
def test_integration_manifest_is_fail_closed_on_claim_scope():
    ...
    assert manifest["rigid_protected_face_clearance"]["physical_validation_eligible"] is False
    assert manifest["rigid_protected_face_clearance"]["rigid_material_policy"] == (
        "NO_RIGID_MATERIAL_INSIDE_RELEASED_PLANAR_HARD_ENVELOPE_XY"
    )
...
def test_candidate_runs_release_export_smoke(tmp_path: Path):
    ...
    assert "waste_cartridge" in report["dfm_gates"]
