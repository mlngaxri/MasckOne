...
def test_recovery_arm_calls_cannot_replace_required_electrical_release_samples() -> None:
    ...
    assert runtime.arm(now_s=fault_at + 0.05) == InputEvent(False, Edge.NONE)
    assert runtime.arm(now_s=fault_at + 0.15) == InputEvent(False, Edge.NONE)
    recovery_deadline = fault_at + runtime.stale_after_s
    assert runtime.arm(now_s=recovery_deadline) == InputEvent(False, Edge.NONE)

    stale = runtime.arm(now_s=recovery_deadline + 0.000001)
