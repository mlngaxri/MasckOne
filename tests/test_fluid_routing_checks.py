...
from masck_one.fluid_routing_checks import (
    ...
    STAGE_WASTE_BACKFLOW_BARRIER_TO_CARTRIDGE,
    STAGE_WASTE_CARTRIDGE_TO_RETENTION,
    STAGE_WASTE_PUMP_TO_BACKFLOW_BARRIER,
...
def built():
    ...
    distribution = build_distribution_geometry_architecture(model.authority, manifold, fresh_pump, water, cleanser, frame, model.coverage_mesh, model.protected_volumes)
    ...
    cartridge = build_waste_cartridge_architecture(model.authority, waste_pump, acquisition, distribution, frame)
    routing = build_fluid_routing_closure_architecture(model.authority, water, cleanser, fresh_pump, manifold, distribution, model.coverage_mesh, model.protected_volumes, acquisition, waste_pump, cartridge, frame)
    return model, frame, water, cleanser, fresh_pump, manifold, distribution, acquisition, waste_pump, cartridge, routing
...
def _validate_current(built, routing):
    ...
    routing.validate_current_sources(authority=model.authority, water=water, cleanser=cleanser, fresh_pump=fresh_pump, manifold=manifold, distribution=distribution, coverage=model.coverage_mesh, protected=model.protected_volumes, acquisition=acquisition, waste_pump=waste_pump, cartridge=cartridge, frame=frame)
...
def test_stage_ledger_matches_controlled_complete_topology(built):
    *_, routing = built
    expected = {
        ...
        STAGE_WASTE_PUMP_TO_BACKFLOW_BARRIER: 1,
        STAGE_WASTE_BACKFLOW_BARRIER_TO_CARTRIDGE: 1,
        STAGE_WASTE_CARTRIDGE_TO_RETENTION: 1,
    }
    actual = {stage: sum(item.stage == stage for item in routing.segments) for stage in expected}
...
def test_route_level_bend_radius_dead_volume_and_clearance_cannot_be_invented(built):
    *_, routing = built
    ...
    for changes in ({"centerline_length_mm": 12.0}, {"inner_diameter_mm": 0.8}, {"minimum_bend_radius_spec_mm": 4.0}, {"realized_minimum_bend_radius_mm": 5.0}, {"dead_volume_mL": 0.01}, {"service_clearance_mm": 2.0}):
...
def test_quantitative_status_cannot_be_spoofed_to_pass(built):
    *_, routing = built
    segment = routing.segments[0]
    with pytest.raises(FluidRoutingClosureError, match="bend-radius status"): replace(segment, bend_radius_status="PASS")
    with pytest.raises(FluidRoutingClosureError, match="dead-volume status"): replace(segment, dead_volume_status="PASS")
    with pytest.raises(FluidRoutingClosureError, match="service-clearance status"): replace(segment, service_clearance_status="PASS")
    with pytest.raises(FluidRoutingClosureError, match="routing topology status"): replace(segment, topology_status="PHYSICALLY_VERIFIED")
...
def test_system_level_quantitative_totals_remain_blocked(built):
    *_, routing = built
    ...
    with pytest.raises(FluidRoutingClosureError, match="total route dead volume"): replace(routing, total_route_dead_volume_mL=0.25)
    with pytest.raises(FluidRoutingClosureError, match="minimum route service clearance"): replace(routing, minimum_route_service_clearance_mm=1.0)
    with pytest.raises(FluidRoutingClosureError, match="quantitative routing closure status"): replace(routing, quantitative_closure_status="PASS")
...
def test_initial_prime_is_carried_only_as_validation_gated_requirement(built):
    *_, routing = built
    ...
    with pytest.raises(FluidRoutingClosureError, match="maximum initial prime status"): replace(routing, maximum_initial_prime_status="VERIFIED")


def test_missing_reordered_mutable_or_duplicated_segment_ledgers_fail_closed(built):
    *_, routing = built
    with pytest.raises(FluidRoutingClosureError, match="stale, incomplete, reordered"): _validate_current(built, replace(routing, segments=routing.segments[:-1]))
    with pytest.raises(FluidRoutingClosureError, match="stale, incomplete, reordered"): _validate_current(built, replace(routing, segments=tuple(reversed(routing.segments))))
    with pytest.raises(FluidRoutingClosureError, match="immutable nonempty tuple"): replace(routing, segments=list(routing.segments))
    with pytest.raises(FluidRoutingClosureError, match="cannot repeat"): replace(routing, segments=routing.segments + (routing.segments[-1],))
...
def test_crossed_or_aliased_segment_interfaces_fail_closed(built):
    *_, routing = built
    ...
    with pytest.raises(FluidRoutingClosureError, match="stale, incomplete, reordered"): _validate_current(built, replace(routing, segments=(crossed,) + routing.segments[1:]))
    with pytest.raises(FluidRoutingClosureError, match="cannot alias"): replace(first, target_interface_id=first.source_interface_id)
...
def test_stale_upstream_hash_authority_revision_and_prime_requirement_fail_closed(built):
    *_, routing = built
    ...
    with pytest.raises(FluidRoutingClosureError, match="maximum initial-prime requirement is stale"): _validate_current(built, replace(routing, maximum_initial_prime_mL=routing.maximum_initial_prime_mL + 0.01))
