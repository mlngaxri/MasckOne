...
def test_export_uses_registry_as_physical_material_boundary(monkeypatch, tmp_path, current_model, registry):
    ...
    monkeypatch.setattr(release_export, "build_structural_frame_topology", lambda *args: SimpleNamespace(manifest=lambda: {}))
    monkeypatch.setattr(release_export, "build_waste_cartridge_dfm_audit", lambda **kwargs: SimpleNamespace(manifest=lambda: {}))
