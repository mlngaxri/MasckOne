...
def test_frame_stays_topology_only_and_whole_frame_blockers_remain_visible():
    ...
    assert "LEFT_RETENTION_TO_FRAME_GEOMETRY_NOT_CURRENT_MAIN_RELEASED" in integration.unresolved_integration
    assert "ACTUATOR_REACTION_CARRIERS_AND_FINAL_MECHANICAL_STOPS_NOT_CURRENT_MAIN_RELEASED" in integration.unresolved_integration
...
def test_active_static_package_uses_split_guide_and_never_reactivates_one_piece_capsule():
    ...
    assembly_ids = {record.assembly_id for record in integration.static_solids}
    ...
    assert len(integration.static_compound.val().Solids()) >= 4
    assert integration.static_compound.val().isValid()


def test_operational_reset_and_factory_states_are_first_class_and_ordered():
    integration = build_mechanical_package_integration()
    assert tuple(state.state_id for state in integration.states) == (
        "LATCHED",
    ...
    assert all(state.full_head_removal_included is False for state in integration.states)
    assert "FULL_POST_RELEASE_WHOLE_HEAD_REMOVAL_TRAJECTORY_OPEN" in integration.unresolved_integration


def test_exact_release_and_factory_motion_breps_are_preserved_with_actual_digests():
    integration = build_mechanical_package_integration()
    motion = {record.assembly_id: record for record in integration.motion_solids}
    ...
    assert exact.brep_sha256 == exact.brep_sha256
...
def test_released_slider_state_is_exact_plus_7p3_mm_rigid_translation():
    integration = build_mechanical_package_integration()
    static = {record.assembly_id: record for record in integration.static_solids}
    motion = {record.assembly_id: record for record in integration.motion_solids}
...
def test_all_integrated_motion_clears_released_fixed_geometry_and_waste_service_envelopes():
    ...
    assert max(check.intersection_volume_mm3 for check in integration.collision_checks) == 0.0
    assert integration.waste_separation_checks
    assert all(check.passes for check in integration.waste_separation_checks)
    assert min(check.separation_mm for check in integration.waste_separation_checks) > 0.0


def test_manifest_and_brep_provenance_are_deterministic_and_physically_fail_closed():
    integration = build_mechanical_package_integration()
    manifest_a = integration.manifest()
    manifest_b = integration.manifest()
    assert manifest_a == manifest_b
    assert len(integration.integration_sha256) == 64
    assert manifest_a["integration_sha256"] == integration.integration_sha256
...
def test_review_export_roundtrips_static_and_exact_motion_geometry(tmp_path: Path):
    integration = build_mechanical_package_integration()
    outputs = export_mechanical_package_review(tmp_path, integration)
    ...
    assert manifest["integration_sha256"] == integration.integration_sha256
