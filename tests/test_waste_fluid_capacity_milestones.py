...
def test_authority_profile_exposes_approaching_capacity_before_failure():
    ...
    thresholds = [m.utilization_fraction for m in evidence.milestones]
    assert thresholds == pytest.approx([.80, .90, 1.0])
...
def test_milestone_evidence_rejects_tampered_cycle():
    evidence = screen_cartridge_capacity_milestones(_guard([20] * 6))
