...
def _sweep() -> LinearSweep:
    return LinearSweep("ACTUATOR_TEST_ENVELOPE", AABB((0.0, 0.0, 0.0), (2.0, 2.0, 2.0), WORLD_FRAME), (10.0, 0.0, 0.0), SOURCE_SHA, True)


def test_continuous_translation_envelope_contains_interior_path_not_only_endpoints():
    sweep = _sweep()
    interior = AABB((5.0, 0.5, 0.5), (5.5, 1.5, 1.5), WORLD_FRAME)
    assert not sweep.start_box.intersects(interior)
    assert not sweep.end_box.intersects(interior)
    assert sweep.collides_with(interior, expected_geometry_sha256=SOURCE_SHA)
    assert sweep.continuous_envelope == AABB((0.0, 0.0, 0.0), (12.0, 2.0, 2.0), WORLD_FRAME)


def test_touching_boundary_and_clearance_are_conservative():
    sweep = _sweep()
    assert sweep.collides_with(AABB((12.0, 0.0, 0.0), (13.0, 1.0, 1.0), WORLD_FRAME), expected_geometry_sha256=SOURCE_SHA)
    near = AABB((12.2, 0.0, 0.0), (13.0, 1.0, 1.0), WORLD_FRAME)
...
def test_coordinate_frame_mismatch_is_hard_failure():
    local = AABB((5.0, 0.5, 0.5), (5.5, 1.5, 1.5), "ACTUATOR_LOCAL_FOREHEAD")
    with pytest.raises(SweepGeometryError, match="coordinate-frame mismatch"):
        _sweep().collides_with(local, expected_geometry_sha256=SOURCE_SHA)


def test_collision_api_cannot_bypass_source_freshness():
    keepout = AABB((5.0, 0.5, 0.5), (5.5, 1.5, 1.5), WORLD_FRAME)
    with pytest.raises(TypeError):
        _sweep().collides_with(keepout)  # type: ignore[call-arg]
    with pytest.raises(SweepGeometryError, match="provenance is stale"):
        _sweep().collides_with(keepout, expected_geometry_sha256="2" * 64)
...
def test_frame_identity_is_preserved_and_explicit():
    ...
    assert sweep.manifest()["coordinate_frame_id"] == WORLD_FRAME
    with pytest.raises(TypeError):
...
def test_sha256_identity_requires_exact_canonical_form():
    alpha = "ab" * 32
    canonical = replace(_sweep(), source_geometry_sha256=alpha)
    assert canonical.source_geometry_sha256 == alpha
    for invalid in (alpha.upper(), f" {alpha}", f"{alpha} "):
        with pytest.raises(SweepGeometryError, match="lowercase canonical SHA-256"):
            replace(canonical, source_geometry_sha256=invalid)
        with pytest.raises(SweepGeometryError, match="lowercase canonical SHA-256"):
            require_fresh_sweep_source(canonical, expected_geometry_sha256=invalid)


def test_manifest_identity_changes_for_mechanical_path_or_frame_change():
    first = _sweep()
    assert first.manifest_sha256 != replace(first, translation_xyz_mm=(10.001, 0.0, 0.0)).manifest_sha256
...
def test_nonfinite_inverted_blank_and_noncanonical_identity_are_hard_failures():
    with pytest.raises(SweepGeometryError, match="finite"):
        AABB((0.0, float("nan"), 0.0), (1.0, 1.0, 1.0), WORLD_FRAME)
    with pytest.raises(SweepGeometryError, match="minimum cannot exceed"):
        AABB((2.0, 0.0, 0.0), (1.0, 1.0, 1.0), WORLD_FRAME)
    for frame in ("", "  ", " MASCK_ONE_WORLD"):
        with pytest.raises(SweepGeometryError, match="canonical string"):
            AABB((0.0, 0.0, 0.0), (1.0, 1.0, 1.0), frame)
    with pytest.raises(SweepGeometryError, match="canonical string"):
        replace(_sweep(), source_id=" ACTUATOR_TEST_ENVELOPE")
...
def test_boolean_and_coercible_aliases_cannot_enter_mechanical_geometry():
    with pytest.raises(SweepGeometryError, match="not a boolean or coercible alias"):
        AABB((True, 0.0, 0.0), (1.0, 1.0, 1.0), WORLD_FRAME)  # type: ignore[arg-type]
    with pytest.raises(SweepGeometryError, match="not a boolean or coercible alias"):
        replace(_sweep(), translation_xyz_mm=("10.0", 0.0, 0.0))  # type: ignore[arg-type]
    with pytest.raises(SweepGeometryError, match="not a boolean or coercible alias"):
        _sweep().collides_with(AABB((20.0, 0.0, 0.0), (21.0, 1.0, 1.0), WORLD_FRAME), expected_geometry_sha256=SOURCE_SHA, clearance_mm=True)  # type: ignore[arg-type]
...
def test_mutable_coordinate_sequences_are_snapshotted_to_immutable_tuples():
    lo = [0.0, 0.0, 0.0]
    hi = [1.0, 1.0, 1.0]
    box = AABB(lo, hi, WORLD_FRAME)  # type: ignore[arg-type]
    before = box.manifest()
    lo[0] = -999.0
    hi[2] = 999.0
    assert box.minimum_xyz_mm == (0.0, 0.0, 0.0)
    assert box.maximum_xyz_mm == (1.0, 1.0, 1.0)
    assert box.manifest() == before

    delta = [10.0, 0.0, 0.0]
    sweep = LinearSweep("MUTABILITY_TEST", box, delta, SOURCE_SHA, True)  # type: ignore[arg-type]
    digest = sweep.manifest_sha256
    delta[0] = 999.0
    assert sweep.translation_xyz_mm == (10.0, 0.0, 0.0)
    assert sweep.manifest_sha256 == digest


def test_wrong_object_types_fail_closed_at_geometry_boundaries():
    with pytest.raises(SweepGeometryError, match="exactly three coordinates"):
        AABB(1.0, (1.0, 1.0, 1.0), WORLD_FRAME)  # type: ignore[arg-type]
    with pytest.raises(SweepGeometryError, match="start_box must be an AABB"):
        LinearSweep("BAD", object(), (1.0, 0.0, 0.0), SOURCE_SHA, True)  # type: ignore[arg-type]
    with pytest.raises(SweepGeometryError, match="Collision geometry must be an AABB"):
        _sweep().collides_with(object(), expected_geometry_sha256=SOURCE_SHA)  # type: ignore[arg-type]
