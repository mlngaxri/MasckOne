...
CENTER_FIELD_PROBE_XY_MM = (0.0, 8.0)
...
def _models():
    ...
    candidate = build_inferior_turnover_exterior_shell(
        authority,
        facial_reference,
        protected,
    ).val()
    return authority, protected, previous, candidate
...
def test_final_brep_side_mass_feathers_progressively_without_moving_package_footprint():
    _, protected, previous, candidate = _models()
    ...
    # The released nostril hard envelopes now intentionally through-cut rigid material at
    # world (0, 0). Keep the visual center-field regression on a nearby stable central
    # datum, and fail closed if future protected-source movement consumes that datum too.
    center_x, center_y = CENTER_FIELD_PROBE_XY_MM
    assert not protected.excluded_xy(Point2(center_x, center_y))
    center_change = abs(
        _front_z_at_xy(candidate, center_x, center_y)
        - _front_z_at_xy(previous, center_x, center_y)
    )
...
def test_final_brep_side_mass_feather_region_meets_absolute_wall_minimum():
    authority, _, _, candidate = _models()
