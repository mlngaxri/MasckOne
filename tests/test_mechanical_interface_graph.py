...
def test_graph_is_bound_to_post_pr121_main_and_canonical_world():
    graph = build_mechanical_interface_graph()
    assert SOURCE_MAIN_SHA == "5a3a07fe4425d55520ed4468c6c71f5bc0a2f45e"
    assert SOURCE_MAIN_TREE_SHA == "9402021a9335bb64c360548649d2cc6a7a1b0fad"
    assert AUTHORITY_REVISION == "2026-08-30-R1"
    assert graph.manifest()["source_main_sha"] == SOURCE_MAIN_SHA
    assert graph.manifest()["source_main_tree_sha"] == SOURCE_MAIN_TREE_SHA
    assert graph.manifest()["frame_id"] == WORLD_FRAME_ID
    assert graph.manifest()["units"] == "mm"
...
def test_exact_candidate_heads_and_blobs_are_pinned_for_review_invalidation():
    ...
    assert candidate_source_heads(graph) == (
        ...
        (92, "1137401b7f25320c4955a8d35cc0f128b6558ab3"),
    ...
    assert sources["REGISTRY_V2"].blob_sha == "d68a109c93532bfa424a574fdbd899230ae20d0b"
    assert sources["FRAME_V1"].blob_sha == "0ea2ada736825fe1a0e06491d16690ae98cfccde"
    assert sources["CARRIER_V1"].blob_sha == "9c613f40ed1b8cb43c3a40bb945d53084a71d121"
    assert sources["ACTUATOR_DONOR_AUDIT_V1"].blob_sha == "d23b574ed0048aba76f64e63471a3327fde2fd5b"
    assert sources["RETENTION_V2"].blob_sha == "9647405b36642105c929a3fdd0617d03bfe68c98"
    assert sources["OCCIPITAL_YOKES_V1"].blob_sha == "6d35c96bc65bb1e0e877deadc65481bf44954b4e"
    assert sources["QUICK_RELEASE_V1"].blob_sha == "11d90a75eb108c53f5a1621abdace7271bf5cac5"
    assert sources["RETENTION_GUARDS_V1"].blob_sha == "b497e9154067cef9ee24da4d421ea6c7861c348e"
    assert sources["SERVICE_INVENTORY_V1"].blob_sha == "e44c8ca12d7b163a5a3fb54fbce7ca2c16d0fc5c"


def test_cell7_donor_audit_is_semantic_evidence_not_mount_geometry():
    audit = source_binding_map()["ACTUATOR_DONOR_AUDIT_V1"]
    ...
    assert all(node.source_id != "ACTUATOR_DONOR_AUDIT_V1" for node in build_mechanical_interface_graph().nodes)


def test_four_carrier_zones_and_local_positive_capture_are_preserved():
    graph = build_mechanical_interface_graph()
...
def test_bilateral_guard_factory_install_paths_fail_closed_on_yoke_collision():
    ...
    for side in ("LEFT", "RIGHT"):
        ...
        assert set(factory.blocking_source_ids) == {
            "RETENTION_GUARDS_V1",
            "OCCIPITAL_YOKES_V1",
        }
