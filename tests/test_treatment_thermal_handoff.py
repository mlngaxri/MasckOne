...
import pytest
from masck_one.thermal_control import ThermalCommand, ThermalCommandInterlock, ThermalInhibitReason, ThermalMode
...
def test_treatment_handoff_exhaustively_preserves_closed_thermal_wire_semantics():
    """Every request pair and both recovery outcomes must serialize to a valid thermal command."""
    permitted = _readiness([0], recovery=1.0, recovery_floor=.95)
    blocked = _readiness([1], recovery=.90, residual=.08, leakage=.02)
    assert permitted.post_recovery_handoff_permitted
    assert not blocked.post_recovery_handoff_permitted

    observed = set()
    for readiness, warm_requested, cool_requested in itertools.product(
        (blocked, permitted), (False, True), (False, True)
    ):
        command = command_thermal_after_treatment(
        ...
        observed.add((
            readiness.post_recovery_handoff_permitted,
            warm_requested,
            cool_requested,
            wire["mode"],
            wire["warm_enable"],
    ...
    assert (False, False, True, "off", False, False, True, "recovery_incomplete") in observed
    assert (True, False, True, "cool", False, True, False, None) in observed
    assert (False, True, False, "warm", True, False, False, None) in observed
    assert (False, True, True, "off", False, False, True, "conflicting_requests") in observed
    assert (True, True, True, "off", False, False, True, "conflicting_requests") in observed
