...
CELL6_HEAD_SHA = "f82e372b43616dfdfd54708c5f228e59c38dfa36"
...
# Live Cell 6 geometry seeds at CELL6_HEAD_SHA.
REACTION_X_MM = 69.0
REACTION_Y_MM = 48.0
RAIL_LENGTH_MM = 6.0
...
# Current treatment-strike seeds. Mirroring the right-side station about X=0 gives
# the left-side candidate and reverses the Y-axis rotation sign so each axis tilts
# laterally outward rather than making both sides tilt wearer-right.
...
# Previous posterior-saddle tube section from the treatment branch.
SADDLE_OUTER_WIDTH_MM = 4.8
SADDLE_OUTER_HEIGHT_MM = 2.0
SADDLE_WALL_MM = 0.35

# Authority/reference loads. These remain validation-gated.
...
# A design allocation, not authority: keep posterior-saddle nominal elastic loss below
# ~10% of full p-p treatment stroke so frame compliance does not consume the control
# margin. Physical stiffness/fatigue must still be measured.
SADDLE_NOMINAL_DEFLECTION_ALLOCATION_MM = 0.05

# Source-bound results from the treatment strike.
CONTROL_CASES = {
    "nominal_1ms": {
        "sensor_delay_ms": 1.0,
        "preload_N": 0.20,
        "peak_force_N": 0.22773206250507494,
    ...
    "ramp_disturbance": {
        "sensor_delay_ms": 1.0,
        "preload_N": 0.20,
        "peak_force_N": 0.25734594282922046,
        "saturation_fraction": 0.0,
        "full_stroke": True,
        "hard_stop_exceeded": False,
    },
    "step_disturbance": {
        ...
        "preload_N": 0.20,
        "peak_force_N": 0.27,
        "saturation_fraction": 0.0418,
        "full_stroke": True,
        "hard_stop_exceeded": False,
    },
    "overload_disturbance": {
        "sensor_delay_ms": 1.0,
        "preload_N": 0.20,
        "peak_force_N": 0.27,
        "saturation_fraction": 0.2868,
        "full_stroke": True,
        "hard_stop_exceeded": False,
    },
    "supervised_step": {
        ...
        "preload_N": 0.20,
        "peak_force_N": 0.27,
        "fault_waveform_withdrawal_s": 1.611,
        "disturbance_start_s": 1.6,
        "disturbed_peak_displacement_mm": 0.20565232394161964,
        "hard_stop_exceeded": False,
    },
    "supervised_overload": {
        "sensor_delay_ms": 1.0,
        "preload_N": 0.20,
        "peak_force_N": 0.27,
        "fault_waveform_withdrawal_s": 1.607,
        "disturbance_start_s": 1.6,
        "disturbed_peak_displacement_mm": 0.35898837748212475,
        "hard_stop_exceeded": False,
    },
}
...
POSITION_OBSERVER_CASES = {
    "1ms_2um_2p7g": {
        ...
        "noise_um": 2.0,
        "mass_g": 2.7,
        "acquisition_peak_mm": 0.47174447555170407,
        "steady_tracking_peak_error_mm": 0.011658326975873573,
        "peak_force_N": 0.27,
    },
    "2ms_5um_2p7g": {
        "sensor_delay_ms": 2.0,
        "noise_um": 5.0,
        "mass_g": 2.7,
        "acquisition_peak_mm": 0.928040442283244,
        "steady_tracking_peak_error_mm": 0.03368894943943548,
        "peak_force_N": 0.27,
    },
}
...
def saddle_section_I_mm4() -> float:
    ...
    return (
        SADDLE_OUTER_WIDTH_MM * SADDLE_OUTER_HEIGHT_MM**3
        - bi * hi**3
    ) / 12.0
...
def required_modulus_MPa(force_N: float, length_mm: float, max_deflection_mm: float) -> float:
    return force_N * length_mm**3 / (
        3.0 * saddle_section_I_mm4() * max_deflection_mm
    )
...
def station_geometry() -> dict[str, dict[str, object]]:
    ...
    for reaction_id, item in STATIONS.items():
        ...
        out[reaction_id] = {
            ...
            "equivalent_contact_pair_force_at_6mm_spacing_N": {
                "continuous": moment_nominal_Nmm / contact_spacing_mm,
                "transient": moment_transient_Nmm / contact_spacing_mm,
            },
        }
    return out
...
def saddle_stiffness(stations: dict[str, dict[str, object]]) -> dict[str, object]:
    worst = max(
        stations.items(),
        key=lambda kv: float(kv[1]["rear_hub_to_reaction_planar_span_mm"]),
    )
    worst_name, worst_item = worst
    ...
    for name, E in materials.items():
        rows[name] = {
            "elastic_modulus_MPa_assumption": E,
            "continuous_z_component_deflection_mm": cantilever_deflection_mm(F_nominal_z, L, E),
            "transient_z_component_deflection_mm": cantilever_deflection_mm(F_transient_z, L, E),
        }
    return {
        ...
        "minimum_modulus_MPa_for_current_section_at_allocation": required_modulus_MPa(
            F_nominal_z, L, SADDLE_NOMINAL_DEFLECTION_ALLOCATION_MM
        ),
        "material_class_studies": rows,
        "decision": (
            "CURRENT_4P8x2P0x0P35_MM_SADDLE_SECTION_REQUIRES_HIGH_MODULUS_INSERT_CLASS_"
            "OR_LARGER_SECTION; UNFILLED_POLYMER_INTEGRAL_SADDLE_REJECTED_FOR_CURRENT_GEOMETRY"
        ),
...
def flexure_decision() -> dict[str, object]:
    ...
    for t, case in FLEXURE_CASES.items():
        ratio = case["radial_k_N_mm"] / case["axial_k_N_mm"]
        mode_sep = abs(case["first_mode_Hz"] - 40.0) / 40.0
        rows[f"{t:.2f}"] = {
            **case,
            "radial_to_axial_stiffness_ratio": ratio,
            "first_mode_fractional_separation_from_40Hz": mode_sep,
            "unpowered_linear_static_deflection_at_0p2N_mm": CONTINUOUS_FORCE_REFERENCE_N / case["axial_k_N_mm"],
        }
    return {
        "cases": rows,
        "selected_study_seed_mm": 0.05,
        "reason": (
            "0P05_MM_RETAINS_ABOUT_390X_RADIAL_AXIAL_STIFFNESS_RATIO_AND_KEEPS_THE_"
            "FIRST_MODE_MATERIALLY_BELOW_40HZ; 0P06_0P07_FORCE_REDUCTION_IS_TOO_COUPLED_"
            "TO_NEAR_RESONANT_BEHAVIOR_FOR_V1_WITHOUT_IDENTIFIED_DAMPING"
        ),
...
def control_decision() -> dict[str, object]:
    ...
    return {
        ...
        "nominal_1ms_force_margin_fraction": (
            ACTUATOR_FORCE_STUDY_LIMIT_N - nominal["peak_force_N"]
        ) / ACTUATOR_FORCE_STUDY_LIMIT_N,
        "ramp_force_margin_N": ACTUATOR_FORCE_STUDY_LIMIT_N - ramp["peak_force_N"],
        "cases": CONTROL_CASES,
        "position_observer_cases": POSITION_OBSERVER_CASES,
        "selected_v1_control_study": (
            "WAVEFORM_SUBTRACTED_DIRECT_POSITION_MULTIRATE_CENTERING_WITH_STABLE_WINDOW_"
            "ACQUISITION_AND_EXPLICIT_WAVEFORM_WITHDRAWAL_SUPERVISOR"
        ),
        "observer_only_status": (
            "REJECTED_AS_V1_BASELINE: 1MS_2UM_CASE_ACQUISITION_PEAK_EXCEEDS_0P45_MM_"
            "CURRENT_HARD_STOP_COORDINATE_IN_THE_MULTIRATE_PLANT"
        ),
        "latency_dependency": (
            "END_TO_END_MEASUREMENT_CONTROL_DELAY_MUST_BE_DEMONSTRATED_IN_THE_1MS_CLASS_"
            "FOR_THIS_ARCHITECTURE; 2MS_CURRENT_MODEL_FAILS_ACQUISITION_AND_SATURATES"
        ),
        "note": (
            "THE 1MS NOMINAL CASE HAS ONLY ~0.042 N PEAK FORCE MARGIN; THE RAMP CASE "
            "HAS ONLY ~0.013 N. STEP/OVERLOAD CASES REACH THE STUDY FORCE CEILING, "
            "SO SUPERVISED WAVEFORM WITHDRAWAL IS ARCHITECTURAL, NOT OPTIONAL."
        ),
        "physical_validation_open": True,
    }
...
def mount_decision() -> dict[str, object]:
    return {
        "selected_reaction_path": [
            "CELL6_KEYED_FRAME_SOCKET",
            "CELL6_EXISTING_KEYED_MALE_REACTION_MATE_AND_SHOULDER",
            "CELL6_TAPERED_CARRIER_RAIL",
            "NEW_CELL7_FEMALE_SLIDE_SHOE_WITH_RIGID_UNDERSIDE_CAPTURE",
            "HIGH_MODULUS_POSTERIOR_SADDLE_INSERT",
            "FIXED_ACTUATOR_REAR_HUB",
            "ACTUATOR",
            "COMPLIANT_OUTPUT_COUPLING",
            "BEARINGLESS_MOVING_TREATMENT_STAGE",
        ],
        "source_change": (
            "DELETE_THE_TREATMENT_STRIKE_REQUIREMENT_FOR_NEW_CENTRAL_DRAW_PIN_BORE_AND_"
            "ASYMMETRIC_KEY_PORT_IN_CELL6; LATEST_CELL6_ALREADY_PROVIDES_A_KEYED_REACTION_"
            "MATE_PLUS_RAIL_PRELOAD_LANDING_AND_DETENT_STACK"
        ),
        "female_counterpart_requirements": {
            ...
            "working_moment_rule": (
                "PRELOAD_LEAF_AND_DETENT_MUST_NOT_CARRY_PRIMARY_MASSAGE_BENDING_MOMENT; "
                "RIGID_CHANNEL_SHOULDERS_AND_REACTION_MATE_MUST"
            ),
        },
...
def build_report() -> dict[str, object]:
    ...
    report = {
        ...
        "source_bindings": {
            ...
            "treatment_head_sha": TREATMENT_HEAD_SHA,
            "treatment_multirate_results_blob_sha": TREATMENT_MULTIRATE_RESULTS_BLOB_SHA,
            "treatment_physics_results_blob_sha": TREATMENT_PHYSICS_RESULTS_BLOB_SHA,
        },
        "authority_references": {
            "treatment_axis_seed_deg": TREATMENT_AXIS_DEG,
            "treatment_stroke_pp_mm": TREATMENT_STROKE_PP_MM,
            "continuous_force_reference_N": CONTINUOUS_FORCE_REFERENCE_N,
            "transient_force_reference_N": TRANSIENT_FORCE_REFERENCE_N,
        },
        ...
        "decisions": [
            "PRESERVE_ACTIVE_SLOW_CENTERING_PLUS_40HZ_WAVEFORM",
            "SELECT_0P05_MM_FLEXURE_AS_CONSERVATIVE_V1_STUDY_SEED_PENDING_NONLINEAR_FEA",
            "REJECT_POSITION_OBSERVER_ONLY_AS_CURRENT_V1_BASELINE",
            "REMOVE_NEW_CELL6_DRAW_BORE_AND_KEY_PORT_REQUIREMENT",
            "CONSUME_EXISTING_CELL6_KEYED_MATE_PLUS_RAIL_PRELOAD_LANDING_DETENT",
            "USE_RIGID_FEMALE_CHANNEL_TO_CLOSE_RX_AND_CARRY_WORKING_MOMENT",
            "KEEP_PRELOAD_LEAF_AND_DETENT_AS_ANTI_RATTLE_CAPTURE_NOT_PRIMARY_REACTION",
            "CURRENT_SUPERIOR_POSTERIOR_SADDLE_SECTION_REQUIRES_HIGH_MODULUS_INSERT_CLASS_OR_SECTION_GROWTH",
            "MIRROR_LEFT_STATION_AXIS_SIGN_SO_BOTH_SIDES_TILT_OUTBOARD",
        ],
        "digital_closure": {
            "four_station_world_seed": "DEFINED_ANALYTICALLY",
            "frame_reaction_source": "BOUND_TO_LIVE_CELL6_HEAD",
            "control_failure_boundary": "QUANTIFIED_FROM_TREATMENT_RESULTS",
            "whole_carrier_brep": "OPEN",
            "whole_carrier_continuous_service_sweep": "OPEN",
            "exact_saddle_joint_stiffness": "OPEN",
            "final_sensor_hardware": "OPEN",
        },
        "physical_validation_required": [
            "ACTUATOR_FORCE_STROKE_AND_COPPER_LOSS",
            "FLEXURE_NONLINEAR_STIFFNESS_STRESS_FATIGUE",
            "SADDLE_JOINT_STIFFNESS_AND_VIBRATION",
            "SENSOR_LATENCY_NOISE_AND_CALIBRATION",
            "CARRIER_INSERTION_PRELOAD_DETENT_FORCE_WEAR_ACOUSTICS",
            "FACIAL_PRELOAD_AND_COMFORT",
        ],
    }
