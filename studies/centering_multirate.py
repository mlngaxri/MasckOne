...
def run(delay_ms=1,noise_um=2,mass_g=2.7,foil_mm=.05,load=.2,seal_k=.02,
        seal_c=.00015,stop_k=4.,disturbance='none',bias_passive=0.,dt=.0001,identified_fixture=False,supervise=False):
    ...
    history=[];ready_history=[];rows=[];rng=np.random.default_rng(5483);r=rv=ra=0.;wave_t=None;stable=0.;phase='ACQUIRE';fault_time=None
    ...
    def waveform(t):
        ...
        if fault_time is not None:
            tau=t-fault_time;Tstop=.05
            if tau>=Tstop:a=da=dda=0.
            elif tau>0:
                f=.5*(1+math.cos(math.pi*tau/Tstop));df=-.5*math.pi/Tstop*math.sin(math.pi*tau/Tstop);ddf=-.5*(math.pi/Tstop)**2*math.cos(math.pi*tau/Tstop)
                a,da,dda=a*f,da*f+a*df,dda*f+2*da*df+a*ddf
    ...
    for n in range(N):
        ...
        if n%hold==0:
            ...
            bias=-ki*integ
            if supervise and wave_t is not None and fault_time is None and (abs(y)>.315 or abs(bias)>.235):
                fault_time=t;phase='WITHDRAW_WAVEFORM'
    ...
    return dict(delay_ms=delay_ms,noise_um=noise_um,mass_g=mass_g,foil_mm=foil_mm,
       supervisor_enabled=supervise,fault_waveform_withdrawal_s=fault_time,identified_fixture_impedance=identified_fixture,inert_preload_N=load,membrane_stiffness_DOE_N_mm=seal_k,membrane_damping_DOE_N_s_mm=seal_c,
...
if __name__=='__main__':
    cases=[{},dict(delay_ms=2),dict(delay_ms=4),dict(load=.1),dict(mass_g=2.2,foil_mm=.045),
      ...
      dict(disturbance='ramp'),dict(disturbance='step'),dict(disturbance='overload'),dict(bias_passive=.1),dict(seal_k=.08,seal_c=.0003,identified_fixture=True),dict(disturbance='step',supervise=True),dict(disturbance='overload',supervise=True)]
