...
def cassette():
    ...
    rear=ring(6.8,7.6,-7.85,-7.525)
    front=ring(1.25,6.4,6.8,7.2)
    rails=[]
    for deg in (0,120,240):
        th=math.radians(deg);x,y=6.35*math.cos(th),6.35*math.sin(th)
        rails += [cylinder(.23,-7.65,7.0).translate((x,y,0)),
          bar((x,y,-7.65),(7.1*math.cos(th),7.1*math.sin(th),-7.65),.2)]
    material['moving_cup']=join([rear,front,ring(1.25,2.8,7.15,7.975),*rails])
    ...
    material['front_stop_ring']=join([ring(3.1,6.4,7.65,7.75),*[bar((6.2*math.cos(math.radians(d)),6.2*math.sin(math.radians(d)),7.7),(7.1*math.cos(math.radians(d)),7.1*math.sin(math.radians(d)),7.7),.12) for d in (60,180,300)]])
    material['front_buffer']=ring(3.1,6.4,7.55,7.65)
