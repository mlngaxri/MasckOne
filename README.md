...
**Fastest scholarship proof route:** start with [three proof points](docs/SCHOLARSHIP_PROOF_POINTS.md). They give the shortest evidence-backed view of what I have actually done, with direct links to inspectable engineering sources and an explicit statement of what each proof point does not establish. For a broader non-technical review, continue to the [90-second scholarship reviewer path](docs/SCHOLARSHIP_REVIEWER_PATH.md), then the [one-page venture snapshot](docs/VENTURE_SNAPSHOT.md). The [scholarship review snapshot](docs/SCHOLARSHIP_REVIEW_SNAPSHOT.md) maps the strongest claims to representative sources and checks. For visual inspection, use the [visual evidence quicklook](docs/VISUAL_EVIDENCE_QUICKLOOK.md): it contains one documented architecture map and three registered candidate digital compositions, explicitly separated from released CAD, photographs and physical evidence. Use the [scholarship evidence matrix](docs/SCHOLARSHIP_EVIDENCE_MATRIX.md) and [evidence guide](docs/EVIDENCE_GUIDE.md) only for deeper audit.
...
*Documentation map of whole-product architecture. It summarises subsystem relationships already documented in the repository so a reviewer can understand the system quickly. Box position, spacing and size are schematic and do not represent physical placement or scale. It does not show implementation status, validated material or control flow, an integrated prototype, or physical validation. Click the diagram to inspect it at full size, then open the [visual evidence quicklook](docs/VISUAL_EVIDENCE_QUICKLOOK.md) for representative registered digital compositions, provenance and explicit evidence limits.*
...
| **What has been built** | Code-generated parametric CAD, system requirements, automated engineering checks, design records and linked subsystem exploration across fit, fluid handling, retention, waste capture, electronics and controls. Exact engineering revision `becf782bf809543ba4b83ecab689204b2458de7d` has a successful GitHub Actions run; that is digital checking evidence for that revision, not physical validation. |
| **Early customer learning** | I ran an informal survey of around 20 people. The strongest reported interest was around time savings and convenience, with concerns around comfort, maintenance and price. This is preliminary and non-conclusive, not evidence of demand or willingness to pay. |
...
| **Current stage** | Pre-commercialisation; engineering geometry exists; digital checks/frameworks exist; product-level physical validation has not begun. There is no integrated manufactured product. |
...
The resulting repository contains system requirements, automated engineering checks, documented design decisions and linked technical exploration across fit, fluid handling, retention, waste capture, electronics and controls. The exact pinned engineering revision has a successful GitHub Actions run, which records that its digital workflow passed. It does not establish physical performance or validate the underlying engineering targets. This chronology demonstrates my initiative and decision ownership; it does not imply that every implementation artefact was authored manually or independently of AI assistance.
