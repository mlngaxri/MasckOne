...
**Fast scholarship-review path:** start with the [one-page scholarship review snapshot](docs/SCHOLARSHIP_REVIEW_SNAPSHOT.md). It maps the strongest reviewer-facing claims directly to representative parametric CAD, controlled requirements, automated checks, documented trade-offs and evidence boundaries. From there, a reviewer can reach representative source/check evidence in one further click. Use the [evidence guide](docs/EVIDENCE_GUIDE.md) only for deeper inspection, and the [visual evidence index](docs/VISUAL_EVIDENCE_INDEX.md) for presentation imagery with provenance and claim limits.
