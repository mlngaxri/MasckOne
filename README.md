...
**Fast scholarship-review path:** start with the [one-page scholarship review snapshot](docs/SCHOLARSHIP_REVIEW_SNAPSHOT.md). It is the canonical reviewer-facing evidence path and maps the strongest claims directly to representative source and checks while stating the evidence boundary beside each claim. For the fastest visual inspection, open the [visual evidence quicklook](docs/VISUAL_EVIDENCE_QUICKLOOK.md): one whole-product architecture diagram plus three traceable candidate digital compositions, with concept imagery excluded and evidence limits stated beside every visual. Use the [scholarship evidence matrix](docs/SCHOLARSHIP_EVIDENCE_MATRIX.md) for a compact claim-to-source audit and the [evidence guide](docs/EVIDENCE_GUIDE.md) only for deeper engineering inspection.
