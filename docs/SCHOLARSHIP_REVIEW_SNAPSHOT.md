...
Accordingly, that run is historical digital evidence for its exact tested revision only. This branch is not presented as a green engineering release. Passing preflights demonstrate that specific digital guards executed successfully at the tested revision; they do not override the failed suite, qualify skipped stages, transfer qualification to later documentation commits, or constitute physical validation. A reviewer can follow the linked run directly rather than relying on a summary detached from its tested revision.
